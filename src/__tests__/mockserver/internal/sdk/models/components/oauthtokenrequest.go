...
type OAuthTokenRequest struct {
	...
	Code *string `form:"name=code" json:"code,omitempty"`
	// Redirect URI (required for authorization_code grant)
	RedirectURI *string `form:"name=redirect_uri" json:"redirect_uri,omitempty"`
	// Client ID (can also be sent via Basic auth header)
	ClientID *string `form:"name=client_id" json:"client_id,omitempty"`
	// Client secret (can also be sent via Basic auth header)
	ClientSecret *string `form:"name=client_secret" json:"client_secret,omitempty"`
	// Refresh token (required for refresh_token grant)
	RefreshToken *string `form:"name=refresh_token" json:"refresh_token,omitempty"`
	// Requested scopes (optional, defaults to original grant scopes)
	Scope *string `form:"name=scope" json:"scope,omitempty"`
	// PKCE code verifier (RFC 7636). Required if code_challenge was used.
	// Must be 43-128 characters from [A-Za-z0-9-._~]
	//
	CodeVerifier *string `form:"name=code_verifier" json:"code_verifier,omitempty"`
}
