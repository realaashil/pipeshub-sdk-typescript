...
import { DoclingProcessPdfServerList } from "../models/operations/docling-process-pdf.js";
...
export function doclingServiceProcessPdf(
  ...
  Result<
    operations.DoclingProcessPdfResponse,
    | PipeshubError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.DoclingProcessPdfRequest$outboundSchema, value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(DoclingProcessPdfServerList[0], {
  ...
  const doResult = await client._do(req, {
    context,
    errorCodes: ["400", "4XX", "500", "504", "5XX"],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.DoclingProcessPdfResponse$inboundSchema),
    M.fail([400, "4XX"]),
    M.fail([500, 504, "5XX"]),
  )(response, req);
  if (!result.ok) {
