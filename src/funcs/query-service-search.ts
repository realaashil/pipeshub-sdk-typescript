...
import { QueryServiceSearchServerList } from "../models/operations/query-service-search.js";
...
export function queryServiceSearch(
  ...
  Result<
    operations.QueryServiceSearchResponse,
    | PipeshubError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.QueryServiceSearchRequest$outboundSchema, value),
  ...
  const baseURL = options?.serverURL
    || pathToFunc(QueryServiceSearchServerList[0], {
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.QueryServiceSearchResponse$inboundSchema),
    M.fail([401, 403, "4XX"]),
    M.fail([500, "5XX"]),
  )(response, req);
  if (!result.ok) {
