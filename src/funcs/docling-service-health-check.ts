...
import { DoclingServiceHealthServerList } from "../models/operations/docling-service-health.js";
 ...
 */
export function doclingServiceHealthCheck(
  client: PipeshubCore,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.DoclingServiceHealthResponse,
    | PipeshubError
...
> {
  const baseURL = options?.serverURL
    || pathToFunc(DoclingServiceHealthServerList[0], {
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.DoclingServiceHealthResponse$inboundSchema),
    M.fail("4XX"),
    M.fail([503, "5XX"]),
  )(response, req);
  if (!result.ok) {
