...
export function organizationsUpdateOnboardingStatus(
  ...
  Result<
    operations.UpdateOnboardingStatusResponse,
    | PipeshubError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.UpdateOnboardingStatusRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.UpdateOnboardingStatusResponse$inboundSchema),
    M.fail([400, 401, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
